...
var (
	...
	DefaultMasterAddress = "http://localhost:8080"
)
...
func (n *NodesList) Set(value string) error {
	*n = append(*n, value)
	return nil
}
...
type GrepParam struct {
	...
	CtxCircle     int            `json:"ctx_circle"`                 // C N — вывести N строк контекста вокруг найденной строки (включает и до, и после; эквивалентно -A N -B N)
	...
	Source        []string       `json:"source" binding:"required"`  // Имя/имена файлов для чтения данных
	...
	RegexpPattern *regexp.Regexp `json:"-"`                          // придется перенести в логику Slave его формирование из Pattern из парсера
}
